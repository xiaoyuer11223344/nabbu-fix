...
import (
	...
	"github.com/xiaoyuer11223344/nabbu-fix/v2/pkg/routing"
...
func sendAsyncTCP4(listenHandler *ListenHandler, ip string, p *port.Port, pkgFlag PkgFlag) {
	...
	if pkgFlag == Syn {
		// flag为syn，则进行构造syn数据包
		tcp.SYN = true
	} else if pkgFlag == Ack {
		// flag为ack，则进行构造ack数据包
		tcp.ACK = true
	}
