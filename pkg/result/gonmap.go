...
type Host struct {
	...
	Status        Status        `xml:"status" json:"status"`                // 开放情况
	Addresses     []Address     `xml:"address" json:"addresses"`            // 地址
	Hostnames     []Hostname    `xml:"hostnames>hostname" json:"hostnames"` // 主机名称
...
type Port struct {
	Protocol string   `xml:"protocol,attr" json:"protocol"` // 端口协议
	PortId   int      `xml:"portid,attr" json:"id"`         // 端口号
